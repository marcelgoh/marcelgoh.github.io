...
the most fundamental laws in discrete mathematics. It is called the {\it pigeonhole principle},
because one formulation of it is, {\sl If $n$ pigeons nest in $n-1$ holes, then there is at least
one hole that contains more than one pigeon.}
First we'll prove a more general statement.
...
This statement can be summed up in one sentence: {\sl The maximum is at least the average.}

A function $f$ is called {\it bijective} (or a {\it one-to-one correspondence})
...
On the other hand, suppose $|A|\ne |B|$. If $A<B$, then $f$ cannot be surjective, since the image of
$f$ has size at most $|A| < |B|$ (at least one element of $B$ must be missed). If $A>B$, then since
$f(A)$ must 
