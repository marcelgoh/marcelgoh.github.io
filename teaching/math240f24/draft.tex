...
\proof We present the proof as a (difficult) exercise. Here is the roadmap.
...
\proof
Enumerate $A = \{a_0, a_2, \ldots\}$ and $B = \{b_0, b_2, \ldots\}$. Now given
...
\proclaim Theorem. The set $\QQ$ of rational numbers is countable.
...
\datestamp{10}{} A {\it relation} on a set $X$ is a subset $R\subseteq X\times X$. If $(a,b)\in R$
we write $aRb$ and say ``$a$ is related to $b$.'' Here are some examples.
\medskip
\item{i)} The set $< = \bigl\{(a,b)\in \RR\times\RR : a<b\bigr\}$ is a relation. For instance, we can
write $(2,\pi)\in <$, or $2<\pi$ (which is the more common notation for this relation).
