...
\medskip\boldlabel Computing large powers modulo {\mathbold p}.
...
\algstep F2. [Compute power.] Set $b\gets a^{n-1} \% n$. (So $0\le b< n$.)
...
Let's see a couple of examples. Suppose we want to find out if $9$ is prime or not (we both know it isn't
...
bits is sufficient.) Then she computes $n=pq$. Now, she picks some integer $k$ with $\gcd(k,n) = 1$.
By Theorem~{\thminvertiblemodn}, there is some integer $s$ such that $ks=1$. Alice publishes
...
$\bar M = M^k\% n$ and sends $\bar M$ to $A$. (We have $0\le \bar M < n$ by the division
algorithm.)
...
Now we shall prove that the encrypted message can be decrypted into the original message.
