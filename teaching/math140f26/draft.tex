...
exists (is a real number), and as we shall see in the next subsection,
...
\proclaim Proposition \advthm. Let
$$f(x) = \cases{-1, & if $x<0$; \cr 1, & if $x\ge 0$}.$$
Then the limit of $f$ at $0$ does not exist.

\proof Let $L$ be arbitrary. We need to show that the limit of $f$ at $0$ is not $L$.
...
$$\bigl| f(-\delta/2) - L\bigr| = \biggl| -1 - L\biggr| \le  \biggl| -1 - 0\biggr|
\ge 1/2 = \eps.$$
On the other hand, if $L$ is negative, we instead choose $x=\delta/2$, so that
