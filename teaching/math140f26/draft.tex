...
A simple application of this theorem is to prove that the function $f(x) = |x|$ goes to $0$ as $x$
approaches $0$. (Try proving this yourself! Show that the one-sided limits both approach $0$, and then
invoke Theorem~{\thmonesidedsame}.)
...
\medskip\boldlabel Infinite limits and vertical asymptotes.
...
has
$$f(x) > M.$$
If, instead, for every large positve number $M$ there is $\delta > 0$ such that every $x$ with
$$a < x < a+\delta$$
satisfies
$$f(x) < -M,$$
...
a valid proof by picking, say, $\delta = 1/M$ instead. (Try it! You'll have to assume that $M>1$, but that
is O.K, since if you can prove the statement for all $M>1$, then those $M$ also trivially satisfy $M>0$.)
Don't stress too much about picking the ``best''
value for $\delta$. The more important thing is to understand that no matter what $M$ is given, you can
...
\smallskip
\item{iv)} $\lim_{x\to a} \bigl( f(x)g(x)\bigr) = \lim_{x\to a} f(x) \cdot \lim_{x\to a} g(x)$; and
\smallskip
\item{v)} $\lim_{x\to a} f(x)/g(x) = \lim_{x\to a} f(x) \bigl/ \lim_{x\to a} g(x)$, provided
that $\lim_{x\to a} g(x)\ne 0$.
\medskip
These identities all hold even when $x\to a$ is replaced with $x\to\infty$ or $x\to-\infty$.

The identites above
are known as (i) the sum law, (ii) the difference law, (iii) the constant multiple law,
(iv) the product law, and (v) the quotient law, respectively.
...
\proof We only prove (i) in the case that $x\to a$ for some finite number $a$ (as opposed to $x\to\infty$).
Laws (ii) through (v) are proved in Appendix~F of the textbook. In the proof of (i) we shall require the
...
$$\bigl| f(x) - L_1\bigr| < {\eps\over 2}
\qquad\hbox{and} \bigl| g(x) - L_2\bigr| < {\eps\over 2}$$
are true as well.
Finally we see that
$$\eqalign{
