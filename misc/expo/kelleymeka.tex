...
\maketitle{Kelley and Meka's proof of Roth's theorem}{by}{Marcel K. Goh}{23 August 2023}
...
$\alpha \le e^{-\Omega(n^{1/9})} = O\bigl(q^{-\Omega(n^{1/9})}\bigr)$.
...
Letting $C$ be the constant implicit in the $O$-notation, we invert this to obtain the bound
$$\alpha\le e^{-n^{1/9}/C^{1/9}} = e^{-\Omega(n^{1/9})},$$
which is what we wanted.\slug
