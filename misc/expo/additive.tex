...
If $\phi : A\to B$ is a $k$-homorphism with an inverse that is also a $k$-homorphism, then we say that
...
Let $G$ be a finite abelian group, let $\chi_1, \ldots, \chi_k$ be characters on $G$, and let
$\delta>0$. The {\it Bohr set} $B(\chi_1,\ldots,\chi_k;\delta)$ is the set
$$\bigl\{ x\in G : \chi_i(x)\in e\bigl([-\delta,\delta]\bigr)\ \hbox{for all}\ 1\le i\le k\bigr\}.$$
(As before, we write $e(\alpha)$ for $e^{2\pi i\alpha}$.) We say that such a Bohr set has {\it dimension} $k$
and {\it width} $\delta$.

If $G= \FF_p^n$, we can write $\chi_i = \chi_{u_i}$ where $\chi_{u_i}(x) = e(u_i\cdot x/p$. If $\delta<1/p$,
...
\parenproclaim Lemma {\advthm} (Bogolyubov). Let $G$ be a finite abelian group. Let $A\subseteq G$ be a
subset of density $\alpha$. Then $2A-2A$ contains a Bohr set of dimension at most $1/\alpha^2$ and width
$1/4$.

\proof For $x\in G$, let
$$\eqalign{
...
Let
$$K = \bigl\{ \chi\in \hat G : \bigl| \hat{\one_A}(\chi) \bigr| \ge \alpha^{3/2}\bigr\}$$
...
< \sum_{\chi\in \hat G} \alpha^3 \bigl| \hat{\one_A}(\chi)\bigr|^2
...
$$\alpha = \sum_{\chi\in \hat G} \bigl| \one_A(\chi)\bigr|^2 \ge
\sum_{\chi\in K} \bigl|\hat{\one_A(\chi)}\bigr|^2 \ge |K|\alpha^3,$$
meaning that $|K|\le 1/\alpha^2$.\slug

When $A$ is a subset of density $\alpha$ in $G = \FF_p^n$, Bogolyubov's lemma tells us that $2A-2A$ contains
