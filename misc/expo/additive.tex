...
a series of YouTube lectures of Timothy Gowers (given in early 2022), though I'm slowly adding more
relevant topics as I learn about them.
\vskip36pt
\noindent{\ninessbx I. Sumsets and the structure of small doubling}
\smallskip
\contline{1}{Pl\"unnecke's theorem}{1}
\smallskip
\contline{2}{The Balog--Szemer\'edi--Gowers theorem}{5}
\smallskip
\contline{3}{Freiman homomorphisms and Ruzsa embedding lemmas}{9}
\smallskip
\contline{4}{Rudiments of discrete Fourier analysis}{12}
\smallskip
\contline{5}{Bohr sets and Bogolyubov's lemma}{17}
\smallskip
\contline{6}{Freiman's theorem in vector spaces over fields of prime order}{19}
\medskip
\noindent{\ninessbx II. Arithmetic progressions}
\smallskip
\contline{7}{Roth's theorem}{22}
\medskip
\noindent{\ninessbx III. Miscellaneous theorems}
\smallskip
\contline{8}{Khovanskii's theorem}{27}
\smallskip
\contline{9}{The Furstenberg--S\'ark\"ozy theorem}{29}
\vskip36pt
...
$rA$ is defined recursively by $0A = \{0\}$ and $rA = A+(r-1)A$.

Note that both $|A+B|$ and $|A-B|$ are trivially bounded above by $|A|\cdot |B|$, since the latter is
the number of ways to choose an element $a$ from $A$ and an element $b$ from $B$.
The trivial lower bound is $\min\bigl\{|A+B|,|A-B|\bigr\}\ge \max\bigl\{|A|, |B|\bigr\}$.
Hence, a sumset $A+B$ can be thought of as large if its size is on the order of $|A|\cdot |B|$,
and small if its size is on the order of $|A|$.

The goal of this first section
is to show that if the cardinality of
a sumset $|A+B|$ is in this smaller regime, then the difference
...
A set $A$ for which the ratio
$|A+A|/|A|$ is small is said to have {\it small doubling}, so Freiman's theorem imposes structure on sets
with small doubling.
...
of size at least $300N/\log\log N$ contains a nontrivial arithmetic progression of length $3$.}

\advsect Solymosi's sum-product bound

[TODO: flesh out this section later.]

The crux of Solymosi's argument lies in the following lemma.
