...
$P$ of length $\lfloor N^{1/3}\rfloor$ and step size at most $2N^{1/100}$
such that $|A\cap P| \ge (\delta + c\delta^3)|P|$, where $c$ does not depend on $\delta$ or $N$.

\proof By hypothesis, $n\in [N]$, we have
...
Recall that at the analogous point in the proof of Roth's theorem, we were able to shift and
...
this reason, we will need another lemma that is suited for iteration.

\proclaim Lemma~\advthm.
Suppose that there is $N\in \NN$ and $A\subseteq [N]$ of density $\delta = |A|/N$ such that
$A-A$ does not contain any positive square numbers. Then there is a set $A'\subseteq [N^{1/4}]$
with $A' \ge (\delta + c'\delta^3) N^{1/4}$, where $c$ is an absolute constant, such that $A'-A'$
also does not contain any squares.

\proof By the previous lemma, there is a progression $P\subseteq [N]$ of length $\lfloor N^{1/3}$
and step size $1\le d \le 2N^{1/100}$ such that $|A\cap P|/|P| \ge \delta + c\delta^3$ for an absolute
constant $c$.
