...
$$2\Eta\{X\} + 2\Eta\{Y\} \le 2\Eta\{Z,W\} + \Eta\{Z\} + \Eta\{W\}.$$
But then since conditioning does not increase entropy, we have
$$2\Eta\{X\} + 2\Eta\{Y\} \le 2\Eta\{Z\} + 2\Eta\{W\},$$
...
Let $X$ and $Y$ be random variables taking values in $\FF_p^*$ for some prime $p$.
There exists $u\in \FF_p^*$ such that
$$\Eta\{X,uY\} \ge {\Eta\{X,Y\}\over 2^{\Etaseven\{X,Y\}}} - 1,$$

\proof Let $U$ be a random variable uniform on $\FF_p^*$ and independent of both $X$ and $Y$.
Independence gives us the identity $\Eta\{X+UY\} = \Eta\{U\} = \log(p-1)$.
We let $(X_1, Y_1)$, $(X_2,Y_2)$, and $U$ be conditionally independent trials
of $(X,Y)$, $(X,Y)$, and $U$ respectively, relative to $X+UY$.
Put
$$H = \Eta\{X_1, Y_1, X_2, Y_2, U\}.$$
Let $W = 1$ if $X_1 = X_2$ and $W=0$ otherwise. Then since $X_1 + UY_1 = X+UY$, we have
$$\eqalign{
H &= \Eta\{X_1, Y_1, X_2, Y_2, W, X+UY\} \cr
&= \Eta\{U\} + \Eta\{W\} + \Eta\{X_1, Y_1, X_2, Y_2 \given W, X+UY\}.\cr
}$$
If $X_1 = X_2$, then $Y_1 = Y_2$, so by conditional independence,
$$\eqalign{
H &= \Eta\{U\} + \Eta\{W\} +  \pr\{X_1 = X_2\given X+UY\} \Eta\{X_1, Y_1 \given X+UY\} \cr
&\qquad+ \pr\{X_1 \ne X_2 \given X+UY\} \Eta\{X_1,Y_1, X_2,Y_2\given X+UY\}.\cr
&= \Eta\{U\} + \Eta\{W\} + \pr\{X_1 = X_2\}\bigl( \Eta\{X,Y\given X+UY\}\bigr) \cr
&\qquad
+\pr\{X_1 \ne X_2\given X+UY\}\bigl( 2\Eta\{X,Y\given X+UY\} \bigr) \cr
&= \Eta\{U\} + \Eta\{W\} + \Eta\{X,Y\given X+UY\}\cr
&\qquad+\pr\{X_1 \ne X_2\given X+UY\}\Eta\{X,Y \given X+UY\} \cr
}$$
But
$$\Eta\{X,Y\given X+UY\} = \Eta\{X,Y, X+UY\} - \Eta\{X+UY\} = \Eta\{X,Y\}$$
by independence of $(X,Y)$ and $U$, so
$$H =\bigl(1+\pr\{X_1\ne X_2\given X+UY\}\}\bigr) \Eta\{X,Y\} + \Eta\{U\} + \Eta\{W\}.$$

On the other hand, by invertibility of each $u\in \FF_p^*$, we have
$$\eqalign{
H - \Eta\{U\} &= \Eta\{X_1, Y_1, X_2, Y_2\given U\} \cr
&= {1\over p-1} \sum_{u\in \FF_p^*} \Eta\{X_1, Y_1, X_2, Y_2\given U=u\} \cr
&= {1\over p-1} \sum_{u\in \FF_p^*} \Eta\{X_1, uY_1, X_2, uY_2\} \cr
&= {1\over p-1} \sum_{u\in \FF_p^*} \AA\{X, uY\}.\cr
}$$
Consequently there exists $u\in \FF_p^*$ such that
$$\eqalign{
\AA\{X,uY\} &\le H-\Eta\{U\} \cr
&= \bigl(1+\pr\{X_1\ne X_2\given X+UY\}\bigr) \Eta\{X,Y\} + \Eta\{W\} \cr
&\le \bigl(1+\pr\{X_1\ne X_2\given X+UY\}\bigr) \Eta\{X,Y\} + 1, \cr
}$$
and for this choice of $u$ we have
$$\Eta\{X+uY\} \ge \pr\{X_1 = X_2\given X+UY\} \Eta\{X,Y\} - 1.$$
But
$$\pr\{X_1 = X_2\given X+UY\} = \pr\bigl\{ (X_1,Y_1) = (X_2, Y_2) \given X+UY\bigr\},$$
and by [TODO: add lemma], we have
$$\eqalign{
\pr\{ (X_1,Y_1) = (X_2, Y_2) \given X+UY\} &\ge 2^{-\Etaseven\{X, Y \given X+UY\}} \cr
&= 2^{-\Etaseven\{X, Y\}},\cr
}$$
which immediately yields the desired inequality.\slug
