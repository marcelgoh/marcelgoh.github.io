...
\maketitle{Entropy and additive combinatorics}{}{Marcel K. Goh}{15 February 2024}
...
have the same sample space. When $X$ and $Y$ are independent, the Ruzsa distance enjoys some
nice properties.
...
$$2\dd\{X,Y\} = 2\Eta\{X-Y\} - \Eta\{X\} - \Eta\{Y\}\over 2,$$
...
We proved earlier that $\P(\FF_2^n)$ is a compact topological space, thus so its Cartesian product with
...
For convenience, we restate what we have just proved with the constants plugged in.
