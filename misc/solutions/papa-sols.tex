...
\proclaim 7. Suppose $f_n : X\rightarrow[0,\infty]$ is measurable for $n=1,2,3,\ldots$, $f_1\geq f_2\geq f_3\geq\cdots\geq 0$, $f_n(x)\rightarrow f(x)$ as $n\rightarrow\infty$ for every $x\in X$, and $f_1\in L^1(\mu)$. Prove that then
...
\proclaim 8. Put $f_n = \chi_E$ if $n$ is odd, $f_n = 1 - \chi_E$ if $n$ is even. What is the relevance of this example to Fatou's lemma?
