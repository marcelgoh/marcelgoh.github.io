...
\maketitle{NDMI011 Combinatorics and Graph Theory}{Notes by}{Marcel K. Goh {\rm (Prague, Czech Rep.)}}{10 June 2019}
...
\proclaim Theorem. In any graph $G=(V,E)$,
...
\thmparens{}{Generalised binomial theorem} For any $r\in \R$,
...
\proclaim Theorem. Let $(a_i)_{i=0}^\infty$ be a sequence of reals such that there exists some $k>0$ for which $|a_i|\leq k^i$ for every $i\geq 1$. Then for all $x$ in the interval $(-1/k, 1/k)$, the power series
...
\proclaim Lemma. For any s-t-flow $f$ and s-t-cut $\delta^+(S)$,
...
\proclaim Theorem. If for all $e\in E$, $c(e)\in \N_0$, then the capacity $n$ of the min-cut is integral and Algorithm M terminates in at most $n$ iterations.
...
\proclaim Theorem. Let $f$ be a max-flow in a network with vertex capacities. If $T$ is a min-cut, then $|f| = d(T)$, and furthermore, if all capacities are integers, then there is an integer max-flow.
...
\thmparens{}{Hall} A bipartite graph $G=(A\cup B, E)$ (with $A$ and $B$ disjoint) has a matching $M\subseteq E$ of size $|M| = |A|$ if and only if $|N(S)|\geq |S|$ for all $S\subseteq A$.
...
\thmparens{}{K\H onig's theorem} Let $G$ be a bipartite graph with maximum matching $M$ and minimum vertex cover $C$. Then $|M| = |C|$.\slug
...
\proclaim Lemma. Let $G=(V,E)$ be a graph. Then for all edges $uv\in E$, $\kappa(G\setminus\{uv\})\geq \kappa(G) - 1$.
...
\proclaim Theorem. A graph is 2-connected if and only if it has an ear decomposition.
...
\proclaim Theorem. Let $e$ be an edge in $K_n$. Then
$$ T(K_n-e) = (n-2)n^{n-3}.$$
...
\proclaim Theorem. The maximum number of edges of any graph $G$ not containing $C_3$ as a subgraph is at most $\lfloor n^2/4 \rfloor$.
...
\proclaim Lemma. Let $(X,{\cal L})$ be a finite projective plane. Then for all $L, L'\in {\cal L}$, there exists an $z\in L$ such that $x\notin L\cup L'$
...
\proclaim Theorem. Let $(X,{\cal L})$ be a finite projective plane of order $n$. Then the following all hold:
\medskip
...
\smallskip
\item {2.} Let $L_1 = \overline{xx_i}$ where $x_i\in L, i\in\{1,\ldots, n+1\}$. By (P1), $L_i\cap L_j = \{x\}$ for all $i\neq j$. As $|L_i\setminus\{x\}| = n$ we have
...
The {\it incidence graph} of a finite projective plane $(X,{\cal L})$, is a bipartite graph $G = (V, E)$ with $V = X\cup {\cal L}$ and $E = \{xL : if x\in L\}$. The concept of duality involves switching the roles of $X$ and ${\cal L}$ in this bipartite graph, interpreting $X$ as lines and ${\cal L}$ as points.
...
\proclaim Theorem. Let $M$ be a set of pairwise orthogonal Latin squares of order $n$. Then $|M|\leq n-1$.
...
\proclaim Theorem. Let $G=(V,E)$ be a graph. If $|V|\geq {k-l-2\choose k-1} = {k+l-2\choose l-1}$, then $\alpha(G)\geq l$ or $\omega(G)\geq k$.
...
$$\Pr[Y] \leq \sum_{A\subseteq V, |A|\geq k} 2^{1-{k\choose 2}} = {n\choose 2}2^{1-{k\choose 2}} \leq {n^k\over k!}2^{1-{k\choose 2}},$$
...
\proof Suppose, towards a contradiction, that every increasing/decreasing subsequence has length at most $r-1$/$s-1$ respectively. Then we label each $x_i$ of $S$ with a pair $(a_i, b_i)$ where $a_i$/$b_i$ is the length of the longest increasing/decreasing subsequence ending in $x_i$. (Note that each of the numbers $x_i, x_j$ have different labels.) Without loss of generality, assume $i<j$. If $x_i\leq x_j$, then $a_i < a_j$, as any increasing subsequence ending in $x_i$ can be extended by $x_j$. Likewise, if $x_j\leq x_i$, then $a_j < a_i$, as any increasing subsequence ending in $x_j$ can be extended by $x_i$.
...
$$C = \{\sum_{i=1}^k \alpha_ix_i : \alpha_i,\ldots \alpha_n\in {\bf F}_q\}.$$
...
Alternatively, represent $C$ by the {\it null space}. Recall that for any $k$-dimensional linear subspace $C$, there is an $(n-k)$-dimensional linear subspace $C^\perp$ such that for all $x\in C, y\in C^\perp$, $x^Ty = 0$. Using the generator matrix $H\in {\bf F}_k^{n\times (n-k)}$ of $C^\perp$, express $C = \{ x : x^TH = 0\}$. Then $H$ is called the {\it parity check} matrix of $C$. In a sense, linear codes are simple because we can detect errors simply by computing $x^TH$.
...
\thmparens{S}{Hamming bound} If an $(n,k,d)_q$-code exists, then
$$q^k\cdot\hbox{vol}\left(\lfloor {d-1\over 2} \rfloor, n\right) \leq q^n.$$

\proof Let $r = \lfloor (d-1)/2 \rfloor$ and consider $\bigcup_{x\in C} B(x,r)\subset \{1,\ldots,q\}^n$. By observation,
$$|\bigcup_{x\in C} B(x,r)| = \sum_{x\in C} \hbox{vol}(r,n) = q^k\cdot \hbox{vol}(r,n) \leq q_n.\noskipslug$$
...
\thmparens{B}{Singleton bound} If $C$ is an $(n,k,d)_q$-code, then $k\leq n-d+1$.

\proof Define a function $f:\Sigma^n\rightarrow\Sigma^{k-1}$ such that $f(x_1,\ldots, x_n) = (x_1,\dots, x_{k-1})$. As $|\Sigma^{k-1}| = q^{k-1}$ and $|C| = q^k > q^{k-1}$, there are $x,y\in C$, with $x\neq y$ and $f(x) = f(y)$. So $x$ and $y$ can only differ in the last $n-k+1$ entries. Since $d = \Delta(C) \leq \Delta(x,y) \leq n-k+1$, $k\leq n-d+1$.\slug
