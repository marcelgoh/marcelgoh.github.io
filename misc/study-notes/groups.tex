...
Let $f:G\rightarrow G'$. The {\it kernel} of $f$, denoted $\ker(f)$, is a the set of all elements in the $G$ that map to the identity $e'$ of the target group $G'$. The {\it image} of $f$, denoted $\Im(f)$, is the set of all elements in $G'$ that equal $f(g)$ for some $g\in G$. Then the homomorphism $f$ is an isomorphism if and only if $\ker(f) = \{e\}$ and $\Im(f) = G'$. It is an easy exercise to verify that $\ker(f)$ and $\Im(f)$ are subgroups of $G$ and $G'$ respectively. In fact, the kernel of a homomorphism is a normal subgroup, as we shall now prove.

\proclaim Theorem K. Let $f:G\rightarrow G'$ be a group homomorphism. Then $K = \ker(f)$ is a normal subgroup of $G$.

\proof By Lemma 2.4.C, we need only show that $K$ is stable under conjugation. Let $k\in K$ be given and let us conjugate it by an element $a\in G$ to get $aka^{-1}$. Applying the homomorphism $f$ to $aka^{-1}$, we find that
$$f(aka^{-1}) = f(a)f(k)f(a^{-1}) = f(a)e'f(a)^{-1},$$
where $e'$ is the identity of $G'$. But $f(a)e'f(a)^{-1} = e'$, so $aka^{-1}\in \ker(f) = K$.\slug
